...
namespace IPO.Dictionary.Services
{
    ...
    {
        public DictionarySearchFileType DictionarySearchFileType => DictionarySearchFileType.Docx;

        public ProcessResults SearchDocument(DictionarySearchProcessModel processModel)
        {
            var result = TryLoadFile(processModel.Data, LoadFormat.Docx, out var document);
        ...
        private MatchingResult SearchNodeForMatch(Node node, IEnumerable<DictionaryValue> dictionaryData)
        {
            var nodeText = node.GetText();
            var match = dictionaryData.FirstOrDefault(value => TextFunctions.TextContainsValue(nodeText, value));
